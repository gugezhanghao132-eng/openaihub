...
class PackagingHiddenImportsTests(unittest.TestCase):
    ...
    def test_macos_build_script_copies_version_file(self) -> None:
        script_path = ROOT / "scripts" / "build-macos.sh"
        script_text = script_path.read_text(encoding="utf-8")
        self.assertIn('cp "$ROOT_DIR/package/version.txt"', script_text)
        self.assertIn('"$DIST_DIR/$OUTPUT_DIR_NAME/version.txt"', script_text)
    ...
    def test_release_workflow_copies_windows_version_file(self) -> None:
        workflow_path = ROOT / ".github" / "workflows" / "build-release-assets.yml"
        workflow_text = workflow_path.read_text(encoding="utf-8")
        self.assertIn("package/version.txt", workflow_text)
        self.assertIn("dist/openaihub/version.txt", workflow_text)
