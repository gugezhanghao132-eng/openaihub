...
OpenAI Hub 是一个给 OpenClAW、OpenCode 和 Hermes 使用的账号池启动器。

它把多个 GPT 账号整理成统一账号池，让你可以用一个入口完成登录、管理、切换和同步，而不是在不同目录和不同工具之间来回手动改配置。

如果你手里有多个 GPT 账号，要轮流给 OpenClAW、OpenCode 或 Hermes 使用，OpenAI Hub 的价值就是把这些重复操作收拢成一次安装、一个命令、一个统一入口。
...
- 维护可切换的账号池
- 一键切换当前账号
- 同步 OpenClAW、OpenCode 和 Hermes 的使用配置
- 在进入主界面前先做环境检查

一句话说清楚：

> OpenAI Hub 不是聊天工具本体，而是给 OpenClAW / OpenCode / Hermes 使用的账号池管理与切号入口。
...
- 把多个 GPT 账号整理成统一账号池
- 用一个入口同时服务 OpenClAW、OpenCode 和 Hermes
- 少做手动改配置、手动换号、手动同步这些重复动作
- 启动前先检查环境，减少因为目录或配置缺失导致的报错

## 适合什么人使用

- 你有多个 GPT 账号，需要轮流给 OpenClAW、OpenCode 或 Hermes 使用
- 你不想每次都手动改配置、手动替换账号
...
### 第三步：确认 OpenClAW / OpenCode / Hermes 已经生成默认目录
...
| 模式 | 会检测什么 | 会切换什么 | 适合谁 |
| --- | --- | --- | --- |
| 综合模式 | OpenClAW + OpenCode + Hermes | 三边一起切 | 三边都在用，想统一管理 |
| OpenCode 模式 | OpenCode | 只切 OpenCode | 主要用 OpenCode，登录可走内置链路 |
| OpenClAW 模式 | OpenClAW | 只切 OpenClAW | 只想管理 OpenClAW |
| Hermes 模式 | Hermes | 只切 Hermes | 只想管理 Hermes |

## 这个产品是干什么的

OpenAI Hub 主要解决的是“一个人手里有多个 GPT 账号，需要把这些账号稳定轮换给 OpenClAW、OpenCode 和 Hermes 去使用”的问题。
...
- 查看当前账号状态
- 在账号之间切换
- 在需要的时候自动把账号切给 OpenClAW、OpenCode 和 Hermes
...
它的作用不是替代 OpenClAW、OpenCode 或 Hermes，而是把你当前账号池里的 GPT OAuth 登录态转成一个本机可调用的 OpenAI 兼容接口。
...
### 3. 如果你要用 Hermes 模式或综合模式，就还需要 Hermes

这两种模式下，程序还会检查 Hermes 的认证文件。

当前检测的默认位置是：

- Hermes 配置目录：`~/.hermes`
- Hermes 凭据文件：`~/.hermes/auth.json`

在 Windows 上，如果你把 Hermes 装在 WSL 中，程序默认会检查：

- `\\wsl$\HermesUbuntu\root\.hermes\auth.json`

如果缺少下面这些关键内容，程序会报错或阻止进入主页面：

- `~/.hermes/auth.json` 不存在
- Hermes 当前认证文件里缺少程序切号所需的 token 字段

默认建议：

- 先正常安装并登录一次 Hermes
- 确保 `auth.json` 已生成
- 如果你在 Windows 上通过 WSL 使用 Hermes，保持默认发行版名 `HermesUbuntu`，或者显式设置 `HERMES_WSL_DISTRO`

## 当前状态

- npm 最新版：`openaihub@1.1.25`
- 安装与更新共用同一条官方源命令
