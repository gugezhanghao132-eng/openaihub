...
- `POST /v1/chat/completions` 面向 OpenAI 兼容客户端
- `POST /v1/messages` 面向 Anthropic / Claude Code 类客户端
- `/v1/messages` 支持 `x-api-key` 鉴权，并要求携带 `anthropic-version` 请求头
- `/v1/messages` 当前支持文本消息、`tools`、`tool_choice`、`tool_use` / `tool_result` 和流式 SSE 桥接
- `/v1/messages` 支持 Claude Code / Claude Haha 发送的截图和 Anthropic `image` 内容块，并会映射到 Codex `input_image`
- 如果你想直接控制 Codex 的推理档位，可以在 Anthropic 兼容请求里传 `reasoning_effort`，或传 `thinking` 让网关映射到 Codex 的 `low` / `medium` / `high` / `xhigh`
